...
seed = 100217
namefile = 'mnar.bin.pois.ip.RData'
nsim = 1000
...
foo = function(miss,vers,cut,misspct,seed){
  ...
  for(i in 1:length(files)){
    ...
    for(j in 1:length(ymiss)){
      ...
      subvar = c('strat','BGid','subid',paste0('W_ip_',miss,'_gfr_',cut,vers,'_',misspct[j]),'strat1','strat2','strat3','strat4',
      ...
      subdat$wts = subdat[[paste0('W_ip_',miss,'_gfr_',cut,vers,'_',misspct[j])]]
