...
seed = 100217
namefile = 'mcar.bin.pois.ip.RData'
nsim = 1000
...
foo = function(miss,vers,cut,misspct,seed){
  ...
  for(i in 1:length(files)){
    ...
    for(j in 1:length(ymiss)){
      ...
      subdat = merge(x=subdat,y=subset(dat.wts,select=c('subid',paste0('W_ip_',miss,'_',misspct[j]))),by='subid',all.x=T)
      
      subdat$response = subdat[[yimp[j]]]
      subdat$baseline = subdat[[paste0('y1_bin_gfr_',cut,'_v3')]]
      subdat$x6imp = subdat[[timp[j]]]
      subdat$wts = subdat[[paste0('W_ip_',miss,'_',misspct[j])]]
